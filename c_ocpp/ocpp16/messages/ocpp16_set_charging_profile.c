...
#include "ocpp16_set_charging_profile.h"
#include <string.h>
...
cJSON *ocpp16_set_charging_profile_req_to_json(const ocpp16_set_charging_profile_req_t *in) {
    ...
    { cJSON *ch = ocpp16_charging_profile_to_json(&in->csChargingProfiles); if (!ch) { cJSON_Delete(obj); return NULL; } cJSON_AddItemToObject(obj, "csChargingProfiles", ch); }
    return obj;
}
ocpp_err_t ocpp16_set_charging_profile_req_from_json(const cJSON *obj, ocpp16_set_charging_profile_req_t *out) {
    ...
    { cJSON *ch = ocpp_json_get_obj(obj, "csChargingProfiles", 1);
      if (ch == NULL) return OCPP_ERR_PARSE;
      if (ocpp16_charging_profile_from_json(ch, &out->csChargingProfiles) != OCPP_OK) return OCPP_ERR_PARSE;
    }
    return OCPP_OK;
}
ocpp_err_t ocpp16_set_charging_profile_req_encode(const ocpp16_set_charging_profile_req_t *in, char *buf, size_t buflen) {
    ...
    cJSON *o = ocpp16_set_charging_profile_req_to_json(in);
...
ocpp_err_t ocpp16_set_charging_profile_req_decode(const char *json, ocpp16_set_charging_profile_req_t *out) {
    ...
    ocpp_err_t rc = ocpp16_set_charging_profile_req_from_json(o, out);
    return rc;
}
void ocpp16_set_charging_profile_req_example(ocpp16_set_charging_profile_req_t *out) {
    memset(out, 0, sizeof(*out));
    out->connectorId = 1;
    ocpp16_charging_profile_example(&out->csChargingProfiles);
}
...
ocpp_err_t ocpp16_set_charging_profile_conf_encode(const ocpp16_set_charging_profile_conf_t *in, char *buf, size_t buflen) {
    ocpp_port_arena_reset();
    cJSON *o = ocpp16_set_charging_profile_conf_to_json(in);
    if (o == NULL) return OCPP_ERR_NOMEM;
    return ocpp_json_dump(o, buf, buflen);
}
ocpp_err_t ocpp16_set_charging_profile_conf_decode(const char *json, ocpp16_set_charging_profile_conf_t *out) {
    ...
    cJSON *o = cJSON_Parse(json);
    if (o == NULL) return OCPP_ERR_PARSE;
    ocpp_err_t rc = ocpp16_set_charging_profile_conf_from_json(o, out);
    return rc;
}
